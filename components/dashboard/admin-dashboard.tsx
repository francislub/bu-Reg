...
import { Printer, Users, UserCog, Building2, BookOpen, ClipboardCheck, FileBarChart2, TrendingUp, ArrowUpRight } from 'lucide-react'
...
export function AdminDashboard({ user, announcements, events: initialEvents }: AdminDashboardProps) {
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      try {
        // Fetch system stats
        const systemStatsResponse = await fetch('/api/analytics/system-stats')
        ...
        setAdminData({
          ...
          departmentsCount: departmentsData.departments?.length || 0,
              ...
              <div className={`bg-gradient-to-br ${card.color} transition-all duration-300 h-full`}>
                ...
                <CardContent className="p-4 pt-2">
                  ...
                  <div className="flex items-center text-xs mt-1">
                    <span className={`flex items-center ${card.trend.startsWith('+') ? 'text-emerald-600' : card.trend === '0%' ? 'text-gray-500' : 'text-rose-600'}`}>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        <Card className="md:col-span-2 shadow-md border-none overflow-hidden">
          <div className="bg-gradient-to-br from-indigo-50 to-blue-50 dark:from-indigo-950/50 dark:to-blue-950/50">
            <CardHeader className="border-b bg-white/50 dark:bg-black/10">
              <CardTitle className="text-lg font-semibold text-indigo-900 dark:text-indigo-100">University Statistics</CardTitle>
        ...
        <Card className="shadow-md border-none overflow-hidden">
          <div className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/50 dark:to-teal-950/50 h-full">
            <CardHeader className="border-b bg-white/50 dark:bg-black/10">
              <CardTitle className="text-lg font-semibold text-emerald-900 dark:text-emerald-100">System Health</CardTitle>
                ...
                <>
                  <div className="space-y-2">
                    ...
                    <Progress value={adminData.systemStats.cpuUsage} className="h-2" 
                      indicatorClassName={adminData.systemStats.cpuUsage > 80 ? "bg-red-500" : adminData.systemStats.cpuUsage > 60 ? "bg-amber-500" : "bg-emerald-500"} />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Progress value={adminData.systemStats.memoryUsage} className="h-2"
                      indicatorClassName={adminData.systemStats.memoryUsage > 80 ? "bg-red-500" : adminData.systemStats.memoryUsage > 60 ? "bg-amber-500" : "bg-emerald-500"} />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Progress value={adminData.systemStats.diskUsage} className="h-2"
                      indicatorClassName={adminData.systemStats.diskUsage > 80 ? "bg-red-500" : adminData.systemStats.diskUsage > 60 ? "bg-amber-500" : "bg-emerald-500"} />
                  </div>
                  
                  <div className="pt-2 text-sm text-center text-muted-foreground">
                    System uptime: {Math.floor(adminData.systemStats.uptime / 24)} days, {adminData.systemStats.uptime % 24} hours
                  </div>
                </>
              )}
            </CardContent>
